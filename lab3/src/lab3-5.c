...
Point3D lightSourcesColorsArr[] = {
  {0.58f, 0.58f, 0.45f}, // Red light
  {0.3f, 0.3f, 0.2f}, // Green light
  {0.3f, 0.3f, 0.2}, // Blue light
  {0.3f, 0.3f, 0.2f} // White light
};
...
void init(void)
{
  ...
  ground_shaders = loadShaders("src/lab3-5-ground.vert", "src/lab3-5-ground.frag");
  skybox_shaders = loadShaders("src/lab3-5-skybox.vert", "src/lab3-5-skybox.frag");
  ...
  LoadTGATextureSimple("models/rutor.tga", &rutor_tex);
  glActiveTexture(GL_TEXTURE2);
  LoadTGATextureSimple("models/grass.tga", &grass_mill_tex);
...
void display(void)
{
  ...
  // Activate textures
  glBindTexture(GL_TEXTURE_2D, rutor_tex);
  glUniform1i(glGetUniformLocation(program, "tex"), 1);
  glBindTexture(GL_TEXTURE_2D, grass_mill_tex);
  glUniform1i(glGetUniformLocation(program, "tex1"), 2);
  printError("millTextures");
